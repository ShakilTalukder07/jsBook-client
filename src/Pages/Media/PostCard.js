...
const PostCard = ({ postInfo }) => {
  ...
  const { data: likes = [] } = useQuery({
    queryKey: ['like'],
    queryFn: async () => {
      const res = await fetch(`https://jssocial-server.vercel.app/like/${_id}`)
  ...
  const handleLick = () => {
    setLike(!like)
    // post like server
    fetch(`https://jssocial-server.vercel.app/like`, {
      method: 'POST',
      headers: {
  ...
  const { data: comments = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`https://jssocial-server.vercel.app/comment/${_id}`)
      const data = await res.json();
      return data;
    }
  })
  ...
  const handleSendMessage = (event) => {
    ...
    fetch(`https://jssocial-server.vercel.app/comment`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json'
      },
